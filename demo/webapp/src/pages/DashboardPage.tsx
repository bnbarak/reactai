...
export const DashboardPage = () => {
  ...
  const [kpi, , kpiRef] = useStateWithAi(
  ...
  const [chart, , chartRef] = useStateWithAi(
  ...
  const [feed, , feedRef] = useStateWithAi(
  ...
  const kpiCards = [
    { label: 'Revenue', value: `$${Number(kpi.revenue).toLocaleString()}`, change: kpi.revenueChange, unit: '%' },
    { label: 'Active Users', value: String(kpi.activeUsers), change: kpi.activeUsersChange, unit: '%' },
    { label: 'Conversion', value: `${kpi.conversionRate}%`, change: kpi.conversionRateChange, unit: 'pp' },
    ...
    <div style={{ padding: 32, fontFamily: 'monospace', maxWidth: 860 }}>
      ...
      {alert.visible && (
        ...
        >
          ...
          <span style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: 1, fontWeight: 'bold' }}>
            {String(alert.type)}
          </span>
        </div>
      )}
        ...
        {kpiCards.map((card) => {
          ...
          return (
            <div key={card.label} style={{ border: '1px solid #ddd', padding: 20 }}>
              <div style={{ fontSize: 11, letterSpacing: 1, textTransform: 'uppercase', color: '#888', marginBottom: 8 }}>
                {card.label}
              </div>
              <div style={{ fontSize: 28, fontWeight: 'bold', marginBottom: 6 }}>{card.value}</div>
              <div style={{ fontSize: 12, color: isPos ? '#2e7d32' : '#c62828' }}>
                {isPos ? '▲' : '▼'} {Math.abs(change)}{card.unit} vs last week
              </div>
            </div>
          )
        })}
      ...
      >
        <div style={{ fontSize: 11, letterSpacing: 1, textTransform: 'uppercase', color: '#888', marginBottom: 16 }}>
          Weekly Activity
        </div>
      ...
      <div ref={feedRef as React.RefObject<HTMLDivElement>}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {(feed.events as Array<{ id: number; text: string; time: string; type: string }>).map((ev) => (
            <div
              key={ev.id}
              style={{
                display: 'flex',
