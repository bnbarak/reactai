...
describe('routes', () => {
  ...
  describe('POST /api/ai/prompt', () => {
    ...
    it('postAiPrompt_withMockedSdk_returnsSdkResult', async () => {
      ...
      const res = await request(app)
        ...
        .send({
          sessionId: session.sessionId,
          prompt: 'make it blue',
          snapshot: [TestUtil.createSnapshot()],
        });
    ...
    it('postAiPrompt_snapshotWithContext_passesEnumToSdk', async () => {
      ...
      const mockSdk = {
        updateFromPrompt: vi.fn().mockImplementation((_prompt, manifests) => {
          ...
          return Promise.resolve({
            target: { key: 'theme-selector', instanceId: 'theme-selector' },
            patch: { selected: 'dark' },
            applied: true,
          });
        }),
      ...
      const schemaProp = (manifest.propsJsonSchema as { properties: Record<string, unknown> })
        .properties;
    ...
    it('postAiPrompt_snapshotWithoutContext_noEnumInSchema', async () => {
      ...
      const mockSdk = {
        updateFromPrompt: vi.fn().mockImplementation((_prompt, manifests) => {
          capturedManifests = manifests;
          return Promise.resolve({
            target: { key: 'title-state', instanceId: 'title-state' },
            patch: { title: 'New' },
            applied: true,
          });
        }),
    ...
    it('postAiPrompt_withMarkersAndUrl_forwardsToSdk', async () => {
      const mockSdk = {
        updateFromPrompt: vi.fn().mockResolvedValue({ applied: true, target: null, patch: null }),
      };
      const app = TestUtil.createApp([], mockSdk);
      const { body: session } = await request(app).post('/api/sessions');
      ...
      await request(app).post('/api/ai/prompt').send({
        sessionId: session.sessionId,
        prompt: 'update it',
        snapshot: [],
        markers,
