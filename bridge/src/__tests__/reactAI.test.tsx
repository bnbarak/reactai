...
vi.mock('../SessionContext.js', () => ({
  useSession: () => ({ sessionId: 'sess-1', serverUrl: 'http://localhost:3001/api' }),
...
describe('reactAI', () => {
  ...
  describe('multiple components', () => {
    ...
    it('threeComponents_patchForOne_onlyUpdatesTargetComponent', async () => {
      ...
      act(() => {
        handlers.forEach((h) =>
          h(TestUtil.createPatchEvent('demo-card', cardInstanceId, { title: 'AI Card' })),
        );
      });
    ...
    it('nestedComponents_patchForChild_doesNotAffectParent', async () => {
      ...
      const AiParent = reactAI(
        ({ theme, children }: { theme: string; children?: React.ReactNode }) => (
          <div data-testid="parent" data-theme={theme}>
            {children}
          </div>
        ),
      ...
      act(() => {
        handlers.forEach((h) =>
          h(TestUtil.createPatchEvent('demo-card', childInstanceId, { title: 'AI Child' })),
        );
      });
