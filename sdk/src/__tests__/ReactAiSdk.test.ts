...
describe('ReactAiSdk', () => {
  describe('updateFromPrompt', () => {
    it('updateFromPrompt_validSelection_returnsApplied', async () => {
      ...
      combinedSelector.select.mockResolvedValue({
        key: 'demo-card',
        instanceId: 'inst-1',
        patch: { title: 'New' },
        done: true,
    ...
    it('updateFromPrompt_validationFails_returnsErrors', async () => {
      ...
      retryValidator.validateWithRetry.mockResolvedValue({
        patch: {},
        errors: ["Prop 'onClick' is not AI-writable"],
      });
...
const createSdk = () => {
  ...
  const combinedSelector = sdkRecord['combinedSelector'];
  const retryValidator = sdkRecord['retryValidator'];
