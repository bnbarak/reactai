...
describe('useStateWithAi', () => {
  ...
  it('useStateWithAi_initial_returnsInitialState', () => {
    const { result } = renderHook(() =>
      useStateWithAi('User profile settings', { username: 'barak' }),
    );
  ...
  it('useStateWithAi_patchEvent_updatesState', () => {
    ...
    act(() => {
      capturedHandler!(
        TestUtil.createPatchEvent('user-profile-settings', 'user-profile-settings', {
          username: 'alice',
        }),
      );
    });
  ...
  it('useStateWithAi_patchEventForDifferentKey_ignoresEvent', () => {
    ...
    act(() => {
      capturedHandler!(
        TestUtil.createPatchEvent('other-component', 'other-component', { username: 'eve' }),
      );
    });
  ...
  it('useStateWithAi_snapshotEvent_replacesState', () => {
    ...
    act(() => {
      capturedHandler!(
        TestUtil.createSnapshotEvent('user-profile-settings', 'user-profile-settings', {
          username: 'fresh',
        }),
      );
    });
  ...
  it('useStateWithAi_stateChange_updatesRegistry', async () => {
    ...
    act(() => {
      result.current[1]({ username: 'alice' });
    });
  ...
  describe('initial state types', () => {
    ...
    it('useStateWithAi_numberValue_returnsNumber', () => {
      const { result } = renderHook(() =>
        useStateWithAi('Counter state', { count: 42, step: 1.5 }),
      );
    ...
    it('useStateWithAi_booleanValue_returnsBoolean', () => {
      const { result } = renderHook(() =>
        useStateWithAi('Toggle state', { enabled: true, visible: false }),
      );
    ...
    it('useStateWithAi_nullValue_returnsNull', () => {
      const { result } = renderHook(() =>
        useStateWithAi('Nullable state', { value: null as unknown as string }),
      );
    ...
    it('useStateWithAi_undefinedValue_returnsUndefined', () => {
      const { result } = renderHook(() =>
        useStateWithAi('Optional state', { label: undefined as unknown as string }),
      );
    ...
    it('useStateWithAi_mixedTypes_registersAllKeysAsAiWritable', async () => {
      renderHook(() => useStateWithAi('Mixed state', { name: 'test', count: 0, active: false }));
