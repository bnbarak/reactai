...
const ProductCardInner = ({
  ...
  >
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      {!inStock && (
        <span
          style={{ fontSize: 11, color: '#aaa', textTransform: 'uppercase', letterSpacing: 0.5 }}
        >
          Out of stock
        </span>
      )}
...
const ProductCard = reactAI(ProductCardInner, {
  key: 'product-card',
  description:
    'An e-commerce product card showing name, category, price, badge, and featured status.',
});

const PRODUCTS: Array<ProductCardProps & { id: string; productKey: string }> = [
  {
    id: 'headphones',
    productKey: 'product-headphones',
        ...
        >
          {PRODUCTS.map((p) => (
            <div
              ...
              style={{
                display:
                  activeCategory === 'All' || activeCategory === p.category ? 'block' : 'none',
              }}
            >
              <ProductCard
                name={p.name}
